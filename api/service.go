...
// allCandidateParents returns every distinct previousTransactionID from a txn's tokens JSONB.
func allCandidateParents(txnID string) []string {
	var rows []struct {
		ParentTxnID string `gorm:"column:parent_txn_id"`
	}
	database.ReadDB.Raw(`
...
// chainDepth follows the first available parent from a txn up to maxDepth hops
// and returns how many hops it could traverse. Used to rank candidates.
func chainDepth(txnID string, maxDepth int) int {
	current := txnID
...
// (up to 5 levels), sorts deepest-chain first, returns top maxParents.
func fetchParents(txnID string, maxParents int) []string {
	candidates := allCandidateParents(txnID)
	if len(candidates) == 0 {
		return nil
	}
	type scored struct {
	...
	for i, c := range candidates {
		list[i] = scored{id: c, score: chainDepth(c, 5)}
	}
